...
import { MAISON_GABRIELLA_CHIEFFO_SUB_PATH } from "../../src/utils/endpoints";
...
export async function getStaticProps({ locale }: { locale: 'it' | 'en'}) {
	const [
		...
		getCategoryPageProps(locale, MAISON_GABRIELLA_CHIEFFO_SUB_PATH)
	]);
	if (!productCategory) {
		return {
			notFound: true
	...
	const products = await getProducts({
		categories: MAISON_GABRIELLA_CHIEFFO_SUB_PATH,
		lang: locale,
		per_page: '99',
		fragrances: true
	})
	...
	const breadcrumbs = [
		...
		{ name: 'Liquides Imaginaires', href: urlPrefix + '/'+MAISON_GABRIELLA_CHIEFFO_SUB_PATH }
	]
	return {
		props: {
			layout: {
