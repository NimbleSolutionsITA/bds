		...
		<div style={{ backgroundColor: 'rgba(0,0,0,0.1)' }}>
			...
			<Container sx={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
				<Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column' }}>
					<Icon sx={{ fontSize: '3rem', marginTop: (theme) => theme.spacing(2) }} />
					<Typography variant="h4" component="h1" sx={{ marginTop: (theme) => theme.spacing(2), textAlign: 'center' }}>
						{isPending ? t('checkout.payment-pending') : t('checkout.payment-success')}
					</Typography>
					<Typography variant="h6" component="h2" sx={{ marginTop: (theme) => theme.spacing(2), textAlign: 'center' }}>
						{isPending ? t('checkout.payment-pending-body') : t('checkout.payment-success-body')}
					</Typography>
...
export async function getStaticProps({ locale }: { locale: LOCALE, params: { id: string } }) {
	const [ssrTranslations] = await Promise.all([
