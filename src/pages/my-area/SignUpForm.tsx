...
export default function SignUpForm() {
	...
	const handleSubmit = (subscribe: (data: EmailFormFields) => void) => (event: React.FormEvent<HTMLFormElement>) => {
		event.preventDefault();
		const data = new FormData(event.currentTarget);
		const values = Object.fromEntries(data);
		if(values.newsletter === 'on') {
			subscribe({ EMAIL: values.email as string })
		}
		register({
			variables: values,
		}).catch(error => {
        ...
        <>
	        <MailchimpSubscribe
		        ...
		        render={({ subscribe, status, message }) => (
			        <form method="post" onSubmit={handleSubmit(subscribe)} style={{padding: '40px 0'}}>
